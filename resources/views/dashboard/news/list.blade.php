...
<div class="row">
    ...
    <div class="col-md-4 col-xs-4">
        <ol class="breadcrumb m-1">
          ...
          <li class="breadcrumb-item active">Listar Notícias</li>
        </ol>
    </div>
</div>
@stop
    ...
    <div class="card-body table-responsive no-padding">
        <div class="row">
            <div class="col-md-12 col-xs-12 text-right p-2">
                <a href="{{ route('news.add') }}" class="btn btn-primary" style="border-radius: 0px !important"><i class="fas fa-plus-circle"></i> Adicionar</a>
            </div>
        </div>
            ...
            @foreach ($data as $item)
                <tbody>
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('news.edit',$item->id) }}" class="btn btn-info"><i class="far fa-edit"></i></a>
                        </td>
                        <td>
                            <a href="javascript:void()" id="button_del_{{ $item->id }}" onclick="deleteRegister({{ $item->id }},{{ $item->status }},'{{ route('news.send.del',$item->id) }}')"  class="btn btn-danger @if($item->status == 0)disabled @endif"><i class="fas fa-trash-alt"></i></a>
                        </td>
                    </tr>
                </tbody>
            @endforeach
