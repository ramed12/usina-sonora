...
<div class="row">
    ...
    <div class="col-md-4 col-xs-4">
        <ol class="breadcrumb m-1">
          ...
          <li class="breadcrumb-item"><a href="{{ route('user') }}">Listar Investidor</a></li>
          <li class="breadcrumb-item active">{{ $action }} Investidor</li>
        </ol>
    </div>
</div>
@stop
...
<div class="card card-outline card-warning">
    <div class="card-header">
        <div class="col-md-12 col-xs-12">
            <a href="{{route('investor') }}" class="card-link"><i class="fas fa-long-arrow-alt-left"></i> Voltar</a>
        </div>
    </div>
    <div class="card-body">
        <form role="form" id="form" method="POST" action="{{ route( $route ) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="password" name="password" id="password" class="form-control @error('password')  is-invalid @enderror" value="@if(!empty($data->password)){{ $data->password }}@endif">
                @error('password')
                    <span class="error invalid-feedback">
                    <strong>{{ $message }}</strong>
                </span>
...
<script>
    $( "#form" ).validate({
        ...
        messages : {
            ...
            password : {
                required : 'O Campo <b>Senha</b> da Imagem é obrigatório.',
            },
            status : {
